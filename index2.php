...
<button type="submit" class="btn btn-primary " data-toggle="modal" data-target="#myModal"></span><i class="fa fa-shopping-bag">    Tambah Transaksi</i></button>
  <!-- <form action="tambah_trans.php"> -->
    ...
    while($d = mysqli_fetch_array($data)){
      ...
      <tr>
        ...
        <td>
        <a href="keranjang2.php?Kode_paket=<?php echo $d['Kode_paket']; ?>" class="btn btn-info"><i class="fa fa-shopping-bag"></i></a>
        </td>
        <?php 
        }    
        ?>
 ...
 <table class="table table-striped">
    ...
    if (isset($_GET['cari'])) {
      ...
      $tgl = date('Y-m-d');
      $user = $_SESSION['Username'];
       $data = mysqli_query($koneksi, "SELECT * FROM transaksi INNER JOIN pegawai ON transaksi.Kode_p=pegawai.Kode_p WHERE pegawai.Username='$user'");
     }
    $nomer=1;
    while($d = mysqli_fetch_array($data)){
      ?>
      <tr>
        ...
        <td>
        <a href="detail2.php?No_trans=<?php echo $d['No_trans']; ?>" class="btn btn-info">Detail</a>
