...
<div class="inner-block">
    <div class="blank">
      <?php
  include 'koneksi.php';
  $query = "SELECT max(Kode_paket) AS maxKode FROM paket";
  $hasil = mysqli_query($koneksi,$query);
  $data = mysqli_fetch_array($hasil);
  $kdpak = $data['maxKode'];
  $nourut = (int)substr($kdpak, 3, 3);
  $nourut++;

  $char = "PL-";
  $kdpak = $char . sprintf("%03s", $nourut);
  ?>

    	<h2><font face="Cambria Math">Data Paket Laundry</font></h2>
<div class="card">
 
  <button type="button" class="btn btn-info" data-toggle="modal" data-target="#myModal"><i class="fa fa-cubes"> +</i></button>
        ...
        <div class="modal-dialog">
          ...
          <div class="modal-content">
            ...
            <div class="modal-header">
              <h4 class="modal-title"><font face="Cambria Math">Input data paket</font>
              <button type="button" class="close" data-dismiss="modal">&times;</button></h4>
            </div>
            <!-- body modal -->
            <div class="modal-body">
       <form action="simpan_dtpaket.php" method="post">
              <font face="Cambria Math"> 
              <table class="table table-hover">
              <tr>
              <td>Kode Paket</td>
              <td><input type="text" size="20" name="Kode_paket" class="form-control" readonly="" value="<?php echo $kdpak; ?>"></td>
              </tr> 
 <?php
      include 'koneksi.php';
       ...
       ?>
              ...
              <td>Kode Outlet</td>
                ...
                <?php while ($d = mysqli_fetch_array($data)) {?>
                  <option value="<?php echo $d['Kode_outlet']; ?>(<?php echo $d['Nama_outlet']; ?>)"><?php echo $d['Kode_outlet'];?>(<?php echo $d['Nama_outlet']; ?>)</option>
                <?php } ?> 
                </select></td>
            ...
            <tr>
              <td>Nama Paket</td>
              <td><input type="text" name="Nama_paket" size="20" placeholder="Masukan Nama" class="form-control" required="required"></td>
              </tr>
              <tr>
              <td>Harga/Kg</td>
              <td><input type="number" name="Harga" size="20" placeholder="Masukan Harga" class="form-control" required="required"></td>
              </tr>  
              <tr>
  ...
  <div class="col-md-9">
    ...
    if (isset($_GET['cari'])) {
       $cari = $_GET['cari'];
       $data = mysqli_query($koneksi, "SELECT * FROM paket where Nama_paket like '%$cari%' or Kode_paket like '%$cari%'");
     } else {
      $user = $_SESSION['Username'];
    ...
    while($d = mysqli_fetch_array($data)){
      ...
      <tr>
        ...
        <td>
          <a href="edit_dtpaket.php?Kode_paket=<?php echo $d['Kode_paket']; ?>" onclick="return confirm('Apakah anda yakin edit?')"><i class='fa fa-edit'></i></a> |
          <a href="hapus_dtpaket.php?Kode_paket=<?php echo $d['Kode_paket']; ?>" onclick="return confirm('Apakah anda yakin hapus?')"><i class="fa fa-trash-o"></i></a>
        </td>
      </tr>
	<?php 
    }
